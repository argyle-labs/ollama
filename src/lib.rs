//! ollama service backend — Local LLM runner.
//!
//! Implements `ServiceBackend` so the generic `service.*` tools
//! (deploy/backup/restore/configure/status/connect/sync) drive ollama. No
...
/// ollama backend. Holds only the provider name; per-instance endpoint/creds
...
impl ServiceBackend for OllamaBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes ollama can be placed on. `service.deploy` hands the
    ...
    {
        // TODO: describe the ollama workload (image/template, ports, mounts,
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("ollama.workload_spec")) })
    }
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("ollama.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("ollama.restore")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply ollama-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("ollama.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("ollama.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = OllamaBackend::new("ollama");
